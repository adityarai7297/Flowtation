...
def create_time_lapse_flow_visualization(returns_df, sector_names, window_units, window_type, step_size, signal_type, temperature=0.8, min_flow_threshold=0.02):
    ...
    # Each entry in animation_frames is a list of traces (edges + nodes) for that frame
    animation_frames = []
    ...
    for period_idx in range(len(period_probabilities)):
        ...
        if period_idx < len(period_probabilities) - 1:
            ...
            # Remove stay-put mass on the diagonal for arrow rendering
            flow_matrix_off = flow_matrix.copy()
            np.fill_diagonal(flow_matrix_off, 0.0)
            ...
            for step in range(interpolation_steps):
                ...
                # Scale a single period flow by easing delta to ensure strict sync
                next_t = min(1.0, t + (1 / interpolation_steps))
                next_eased_t = 3 * next_t**2 - 2 * next_t**3 if next_t < 1 else 1
                delta_eased = max(0.0, next_eased_t - eased_t)
                frame_flow_matrix = flow_matrix_off * delta_eased
                ...
                # Add this frame's combined traces
    ...
    frames = []
    for frame_idx, frame_traces in enumerate(animation_frames):
